...
public:
	...
	void player_stand();
	/*
		功能：遍历小孩kid_v,指挥（player_x - width , player_x + width ）范围内的小孩，修改其flag为move 1
	*/
	void move_command(vector<Kids>& kid_v, Player joe);

	/*
		功能：遍历小孩kid_v,指挥（player_x - width , player_x + width ）范围内的小孩，修改其flag为stop 2
				若小孩本来就是wait ，重置对应(kid_timer)
	*/
    void stop_command(vector<Kids>& kid_v, Player joe);

	/*
		功能：被车撞了，玩家的x坐标发生变化   怎么变？
	*/
	void player_be_hit();

	/*
	功能：
		当玩家player_flag 为left 3的时候 ，单位时间内玩家向左移player_speed 的距离，若player_flag为right 4的时候， 单位时间向右移动player_speed的距离
	...
	*/
	...
	//-------------------------------view---------------------------------------------
	/*
		功能：结合 玩家player_x 根据对应flag展示对应动作
	*/
	void player_view();
