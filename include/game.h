 ...
 * 			左侧骑车到 汽车生成间隔 TL
 * 			中侧汽车道 汽车生成间隔 TM
 * 			右侧汽车道 汽车生成间隔 TR * 			三条道路 lane_left -1、lane_mid 0、lane_right 1。
 *
 *
 * 		2.函数设计
 * 			GameInit:初始化
...
/**
 * @brief 功能：实时展示玩家分数，展示score在对应位置
 */
void GameScoreView();

/**
 * @brief 功能：实时展示死亡小孩数，展示deadnum在对应位置
 */
void GameDeathView();

/**
 * @brief 功能：展示还没上场小孩数，展示kidnum在对应位置
 */
// void game_wait_view();

/**
 * @brief 功能：展示通过小孩数量passnum到对应位置
 */
void GamePassView();
