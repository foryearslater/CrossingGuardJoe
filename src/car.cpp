...
void Cars::car_route() {
    if (this->car_lane == -1) {
        while (this->car_y >= 300 && this->car_y <= 550)//限定纵坐标范围
        {
            this->car_x = car_x1 - (this->car_y - 200) / 5;//斜线函数
            //之后car_x保持不变
            return;
        }
    }
        ...
        while (this->car_y >= 300 && this->car_y <= 550)//限定纵坐标范围
        {
            this->car_x = car_x3 + (this->car_y - 200) / 5;//斜线函数
            //之后car_x保持不变
            return;
        }
    }
...
void Cars::car_view() {
    ...
    putimage(this->car_x, this->car_y, car_body, NOTSRCERASE);//掩码图
    putimage(this->car_x, this->car_y, car_body + 1, SRCINVERT);//原图
